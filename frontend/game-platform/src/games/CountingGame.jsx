...
// Counting game - shows image with objects, user picks correct number video
...
export default function CountingGame(props) {
  ...
  // Filter valid signs with counting images
  const getValidSigns = (items) => {
    return items.filter(sign => 
      sign.name && 
      sign.videoUrl && 
      sign.assets?.count?.images && 
      sign.assets.count.images.length > 0
    );
  };
  ...
  const pickRandomSignWithImage = (items) => {
    ...
    const available = valid.filter(sign => {
      return sign.assets.count.images.some(imgObj => {
        const imgPath = Object.values(imgObj)[0];
        return !usedImages.has(imgPath);
      });
    });
    ...
    if (available.length === 0) {
      ...
      const randomImgObj = randomSign.assets.count.images[
        Math.floor(Math.random() * randomSign.assets.count.images.length)
      ];
    ...
    const unusedImages = randomSign.assets.count.images.filter(imgObj => {
      const imgPath = Object.values(imgObj)[0];
      return !usedImages.has(imgPath);
    });
  ...
  const startRound = () => {
    ...
    if (validSigns.length < 4) {
      console.error("Need at least 4 signs with counting images");
      return;
    }
    ...
    // Extract image path and label from imageObj
  ...
  if (!validSigns || validSigns.length < 4) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50">
        <p className="text-red-600 font-semibold">Need at least 4 numbers with counting images for this game.</p>
      </div>
    );
  }
        ...
        <div className="flex flex-col items-center min-h-full max-w-5xl mx-auto">
          ...
          {showInstructions && (
            <div className="text-xs sm:text-sm bg-white/80 backdrop-blur-md shadow-xl p-3 sm:p-4 rounded-xl sm:rounded-2xl mb-2 sm:mb-3 w-full border border-white/60 animate-in fade-in slide-in-from-top duration-300">
              <p className="mb-2 font-semibold text-gray-700">Count the objects in the image and pick the correct number sign!</p>
              <ul className="text-xs text-gray-600 space-y-1 list-disc list-inside">
                <li>Look at the image and count the objects</li>
                <li>Watch the 4 video options showing different numbers</li>
                <li>Click on the video that shows the correct count</li>
            ...
            <>
              ...
              <div className="mb-3 sm:mb-4 w-full">
                <div className="bg-white/80 backdrop-blur-sm shadow-lg p-3 sm:p-4 rounded-xl sm:rounded-2xl border-2 border-white/60">
                  <p className="text-sm sm:text-lg md:text-xl font-bold text-center text-gray-800 flex items-center justify-center gap-2">
                    <Search size={20} className="sm:w-6 sm:h-6" style={{ color: colors.primary }} />
                    <span className="text-sm sm:text-base md:text-lg">Count the {imageLabel} and pick the correct number</span>
                  </p>
                </div>
              </div>
              ...
              <div className="mb-4 sm:mb-6 w-full flex flex-col items-center">
                <div className="bg-white/80 backdrop-blur-md rounded-xl sm:rounded-2xl shadow-2xl border-4 border-white/50 p-3 sm:p-4 max-w-lg w-full">
                  ...
                  />
                  <p className="text-center mt-2 sm:mt-3 text-base sm:text-lg font-semibold text-gray-700 capitalize">
                    How many {imageLabel}?
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white/80 backdrop-blur-md rounded-xl sm:rounded-2xl p-6 sm:p-8 shadow-2xl border border-white/60 mb-4 sm:mb-6">
                ...
                <div className="mt-4 sm:mt-6 p-3 sm:p-4 bg-gray-50 rounded-xl space-y-3">
                  ...
                  <p className="text-lg sm:text-xl font-bold text-gray-800 mt-2">
                    There {currentQuestion.name === "One" ? "is" : "are"} <span className="text-2xl" style={{ color: colors.primary }}>{currentQuestion.name}</span> {currentQuestion.name === "One" ? "object" : "objects"} in the image
                  </p>
                </div>
              </div>
