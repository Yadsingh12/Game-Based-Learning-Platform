...
export default class SignMatchScene extends Phaser.Scene {
  ...
  create() {
    ...
    const exitButton = this.add.text(
      ...
      20,
      'Exit',
      { fontSize: '48px', color: '#ff0000', backgroundColor: '#ffffff' }
    ...
    // estimate how many rows needed
    const gridRows = Math.ceil(this.cardsData.length / gridCols);

    const availableWidth = this.scale.width;
    const availableHeight = this.scale.height - this.uiPanelHeight;

    const idealTotalWidth = gridCols * this.cardOriginalWidth + (gridCols+1) * spacingX;
    const idealTotalHeight = gridRows * this.cardOriginalHeight + (gridRows+1) * spacingY;

    let scaleFactor = 1;

    if (idealTotalWidth > availableWidth || idealTotalHeight > availableHeight) {
    ...
    const totalWidth = gridCols * this.cardWidth + (gridCols - 1) * spacingX;
  ...
  createCardContainer(x, y, cardData) {
    ...
    const back = this.add.image(0, 0, 'card_back')
  ...
  flipCard(card) {
    ...
    if (type === 'image') {
      const frontImage = this.add.image(0, 0, textureKey)
        .setDisplaySize(this.cardWidth, this.cardHeight);
      card.add(frontImage);
